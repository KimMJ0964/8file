/*
    ...
    (테이블에 이벤트가 발생 했을 때)
    자동으로 매번실행할 내용을 미리 정의해둘 수 있다.
    
     EX)
     회원탈퇴시 기존의 회원 테이블에 데이터 DELETE한 후 곧바로 탈퇴한 회원들만 따로 보관하는 테이블에
     자동으로 INSERT 시켜야한다.
     신고횟수가 일정 수를 넘었을 때 묵시적으로 해당 회원을 블랙리스트로 처리하게끔
     입출고에 대한 데이터 기록(INSERT)될때마다 해당 상품에 대한 재고수량을 매번 수정(UPDATE)해야한다
...
--EMPLOYEE 테이블에 새로운 행이 INSERT될때 마다 자동으로 출력되는 트리거 정의
...
VALUES(SEQ_EID.NEXTVAL, '김용담', '111111-2111111', 'D7', 'J7', SYSDATE);
...
CREATE TABLE TB_PRODUCT(
    ...
    PNAME VARCHAR2(30) NOT NULL, --상품 명
...
INSERT INTO TB_PRODUCT VALUES(SEQ_PCODE.NEXTVAL, '대륙폰', '샤오미', 8000000, 20);
...
--TB_PRODUCT 테이블에 매번 자동으로 재고수량을 UPDATE하게끔 트리거 작성
...
/*
    ...
    -상품이 출고된경우 -> 해당 상품을 찾아서 재고수량 감소 UPDATE
     UPDATE TB_PRODUCT
    SET STOCK = STOCK - 입고된 수량 (INSERT된 자료의 AMOUNT)
    WHERE PCODE = 입고된 상품번호(INSERT된 자료의 PCODE);
*/
