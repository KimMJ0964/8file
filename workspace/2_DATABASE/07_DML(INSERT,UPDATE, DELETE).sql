...
/*
    ...
    부족하게 값을 제시할 경우, 값을 더 많이 제시한경우 -> 에러
*/
SELECT * FROM EMPLOYEE;

INSERT INTO EMPLOYEE
VALUES(900, '이소근', '880914-1456789', 'LSG7194@NAVER.COM','01077047194','D7',
...
VALUES(901, '김용담', '000908-1234567', 'J7', SYSDATE);
...
--전체사원의 급여를 기존 급여의 10프로 인상된 금액으로 변경해라
UPDATE EMP_SALARY
...
UPDATE EMP_SALARY
    SET BONUS = 0.3
WHERE EMP_ID IN (SELECT EMP_ID, EMP_NAME
                    FROM EMP_SALARY
                    JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
                    JOIN LOCATION ON(LOCATION_ID = LOCAL_CODE)
                    WHERE LOCAL_NAME LIKE 'ASIA%');   
...
COMMIT;
/*
    4.테이블에 기록된 데이터를 삭제하는 구문 (한 행 단위로 삭제가 된다.)
    [표현식]
    DELETE FROM 테이블명
    [WHERE 조건] - > WHERE 절 제시 안아면 전체행 다 삭제됨
