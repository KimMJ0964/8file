...
FROM EMPLOYEE;
--산술연산 과정중에 NULL 데이터가 포함되어있다면 무조건 결과값은 NULL
...
/*
    <DISTINCT>
    중복제거 - 컬럼에 표시된값들을 한번씩만 조회하고자 할 때 사용
*/
--EMPLOYEE 의 직급코드 조회
...
--위처럼 작성시 에러가 발생한다. DISTINCT는 한번만 사용가능하다.
...
-----------------실습-------------------
--1. 급여가 300만원 이상인 사람들의 사원명, 급여, 입사일, 연봉(별칭 -> 연봉)조회
...
/*
    ...
    특정패턴을 제시할 때 와일드 카드라는 특정패턴이 정의되어야한다.
    1. '%' : 포함문자 검색(0글자 이상 전부 조회)
     EX) 비교할 대상 컬럼 LIKE '문자%' : 비교할 대상 컬럼값 중에서 해당문자로 시작하는 값들만 가져온다
         비교할 대상 컬럼 LIKE '%문자' : 비교할 대상 컬럼값 중에서 해당문자로 끝나는 값들만 가져온다
        비교할 대상 컬럼 LIKE '%문자%' : 비교할 대상 컬럼값 중에서 해당문자로 포함된 값 조회
    ...
    2.'_' : 1글자를 대체하는 검색
     EX) 비교할 대상 컬럼 LIKE '_문자' : 비교할 대상 컬럼값 문자앞에 아무글자나 한글자가 있는 값을 조회
        ...
        비교할 대상 컬럼 LIKE '__문자___' : 내가 원한느 형태로 _를 총해서 문자수를 조절할 수 있다.
...
--2. 전화번호 처음 3자리가 010이 아닌사원들의 사원명, 전화번호 조회
SELECT EMP_NAME, PHONE
FROM EMPLOYEE
WHERE PHONE NOT LIKE '010%';
--3. 이름에 '하'가 포함되어있고 급여가 240만원이상인 사원들의 사원명, 급여조회
...
--2. 연봉(보너스 미포함)이 3천만원 이상이고 보너스를 받지않는 사원들의 사번, 사원명, 급여, 보너스 조회
...
FROM EMPLOYEE
WHERE HIRE_DATE >= '90/01/01' AND DEPT_CODE IS NULL;
--4. 급여가 200만원 이상이고 500만원 이하인 사원중에서 입사일이 '01/01/01'이상이고 보너스를 받지않은사원들의
--사번, 사원명, 급여, 입사일, 보너스 조회
SELECT EMP_ID, EMP_NAME, HIRE_DATE, DEPT_CODE
FROM EMPLOYEE
WHERE HIRE_DATE >= '01/01/01' AND (SALARY >= 2000000 OR SALARY <= 5000000) AND BONUS IS NULL;
--5. 보너스를 포함 연봉이NULL이 아니고 이름에'하'가 포함된 사원들의 사번, 사원명, 급여, 보너스포함 연봉 조회
SELECT EMP_ID, EMP_NAME, SALARY, SALARY + (SALARY * BONUS)
FROM EMPLOYEE
