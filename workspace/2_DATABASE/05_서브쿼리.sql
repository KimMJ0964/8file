/*
    *서브쿼리
    -하나의 SQP 문 안에 포함된 또 다른 SELECT문
    -메인 SQL 문을 위해 보조 역할을 하는 쿼리
*/

--서브쿼리예시1)노옹철 사원과 같은 부서에 속한 사원들조회
...
/*
    ...
    --다중행 다중열 서브쿼 : 서브쿼리의 조회 결과값이 여러행 여러컬럼일 때
...
--6) 전지언 사원과 같은 부서의 사람들의 사번, 이름, 전화번호, 입사일, 부서명 조회
--단 전지언 사원 제외
...
--1_1) 유재식 또는 윤은해 사원의 직급
SELECT JOB_CODE
FROM EMPLOYEE
WHERE EMP_NAME IN('유재식','윤은해');

--1_2) 직급이 J3, J7 인 사원들의 사번, 사원명, 직급코드 급여조회
SELECT EMP_ID, EMP_NAME, JOB_CODE, SALARY
FROM EMPLOYEE
...
--2) 대리 직급임에도 과장 직급 급여들 중 최소 급여보다 많이 받는 사원의 사번, 이름, 직급, 급여 조회
--2_1) 과장 직급 급여들
SELECT SALARY
FROM EMPLOYEE
...
--2_2)대리직급이면서 위의 결과값보다 하나라도 큰사원
SELECT EMP_ID, EMP_NAME, JOB_CODE, JOB_NAME, SALARY
FROM EMPLOYEE
LEFT JOIN JOB USING(JOB_CODE)
WHERE  SALARY > 
                ANY(3760000,2200000,2500000)
                AND JOB_NAME = '대리';
...
SELECT EMP_NAME, EMP_NAME, JOB_CODE, HIRE_DATE, MANAGER_ID
FROM EMPLOYEE
...
--> 각 직급별 최소급여를 받는 사원조회(사원, 사원명, 직급코드, 급여)
SELECT EMP_ID, EMP_NAME, JOB_CODE, SALARY
FROM EMPLOYEE
...
WHERE (DEPT_CODE, SALARY) IN(SELECT DEPT_CODE, MAX(SALARY)
                                ...
                                GROUP BY DEPT_CODE);
--==================================================================
--1.부서 별 급여 합계가 전체 급여 총 합의 20%보다 많은 부서의 부서 명, 부서별 급여 합계조회
...
SELECT EMP_ID, EMP_NAME,
    ...
    END + 1 AS "나이", DEPT_TITLE, JOB_NAME
FROM EMPLOYEE
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
JOIN JOB USING(JOB_CODE)
...
--단, 보너스 포함 연봉이 3000만원 이상인 사람들만 조회

--ROWNUM은 오라클에서 제공하는 컬럼, 조회된 순서대로 1부터 순번을 부여한다.
...
FROM(SELECT EMP_NAME, SALARY
            ...
            ORDER BY SALARY DESC)
            WHERE ROWNUM <= 5;
--FROM절에 있는 SELECT는 전체 SALARY를 순번대로 정리해준것이고 풀어보면
--사원이름과 급여를 전체 SALARY를 순번대로 정리해준것 중 ROWNUM 이 5개까지
--ROWNUM : 나오는 순서대로 정렬해주는것 순서대로
