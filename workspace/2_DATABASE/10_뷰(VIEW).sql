/*
    ...
    임시 테이블 같은 존재(실제 데이터가 담겨있는건 아니다 -> 논리 테이블)
*/
...
/*
    *뷰 컬럼에 별칭 부여
    서브쿼리의 SELECT 절에 함수식이나 산술연산식이 기술되어있다면 반드시 별칭을 부여해야한다.
*/
...
/*
    *DML 명령어로 조작이 불가능한 경우가 많다
    ...
    2) 뷰에 정의되어있지 않은 컬럼 중에 베이스 테이블 상에 NOT NULL제약조건이 지정되어있는 경우
...
/*
    ...
    [상세표현식]
    CREATE [OR REPLACE] [FORCE | NOFORCE] VEIW 뷰명
    AS 서브쿼리
    [WITH CHECK OPTION]
    [WITH READ ONLY];
    ...
    2) FORCE| NOFORCE
        ...
        > NOFORCE : 서브쿼리에 기술된 테이블이 존재하는 테이블이여만 한다(기본값)
    3)WITH CHECK OPTION : DML시 서브쿼리에 기술된 조건에 부합한 값으로반 DML이 가능하도록
...
--200번 사원 급여를 200만원으로 변경(SALARY >= 3000000 조건에 부합하지 않는 변경)
UPDATE VW_EMP
SET SALARY = 2000000
WHERE EMP_ID = 200;
...
--200번 사원 급여를 200만원으로 변경(SALARY >= 3000000 조건에 부합하지 않는 변경)이 되지 않는다.
UPDATE VW_EMP
SET SALARY = 2000000
WHERE EMP_ID = 200;
