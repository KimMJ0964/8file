bSELECT EMP_ID, EMP_NAME, SALARY
FROM EMPLOYEE
...
/*
    ...
    FROM 조회할 테이블
    WHERE 조건식
    ORDER BY 정렬기준될 컬럼 | 별칭 | 컬럼의 순번 [ASC | DESC][NULLS FIRST | NULLS LAST]
...
--정렬기준에 컬럼값이 동일할 경우 그 다음차순을 ㅜ이해서 여러개를 제시할 수 있다.
...
/*
    ...
    >>SELECT 절에 단일행함수랑 그룹합수를 함께 사용하지 못함
    ->결과 행의 갯수가 다르기 때문
...
SELECT SUBSTR('SHOWMETHEMONEY', 7) FROM DUAL; -- 7번째 위치부터 끝까지 호출
...
/*
    *LTRIM/ RTRIM
    ...
    문자열의 왼쪽혹은 오른쪽에서 제거하고자하는 문자들을 찾아서 제거한 나머지 문자열 반환
...
SELECT TRIM(LEADING 'Z' FROM 'ZZZKHZZZZZZZZ') FROM DUAL; -- 양쪽에 있는 공백제거
SELECT TRIM(TRAILING 'Z' FROM 'ZZZKHZZZZZZZZ') FROM DUAL;
...
/*
    *LOWER / UPPER / INITCAP
    ...
    INITCAP : 띄어쓰기 기준 첫글자마다 대문자로 변경할 문자열 반환
*/
...
SELECT ROUND(123.456, 1) FROM DUAL;--양수로 증가할수록 소수점뒤로 한칸씩이동
...
SELECT EMP_NAME, HIRE_DATE, ADD_MONTHS(SYSDATE, 3)
FROM EMPLOYEE;
...
/*
    [형변환 함수]
    *TO_CHAR :숫자타입 또는 날짜 타입의 값을 문자타입을 변환시켜주는 함수
...
SELECT TO_CHAR(12,'00000') FROM DUAL;---0의 자리수만큼 공간 확보, 빈칸을 0 으로 채움
...
SELECT TO_cHAR(SYSDATE, 'YYYY'),
       TO_cHAR(SYSDATE, 'YY'),
       TO_cHAR(SYSDATE, 'RRRR'),-- RR룰이 따로 존재한다 -> 50년 이상값이 + 100 EX)1954 -> 2054
...
SELECT TO_DATE(240807) FROM DUAL; -- 50년 미만은 자동으로 20XX으로 설정 50년 이상은 19XX로 설정된다.

SELECT TO_DATE(020505) FROM DUAL; --숫자는 0으로 시작할 수 없다
SELECT TO_DATE('020505') FROM DUAL;--이렇게 바꾸어주면 된다
...
/*
    ...
    TO_NUMBER(문자, [포맷])
*/
SELECT TO_NUMBER('051234567890') FROM DUAL;
...
--*NULLIF(비교대상1, 비교대상2)
--두 값이 일치하면 NULL 일치하지 않으면 비교대상1이 반환
...
--COUNT(컬럼) : 제시한 해당 컬럼 값이 NULL이 아닌것만 행의 수를 세서 반환
...
((SALARY +(SALARY*NVL(BONUS,0)))*12 -((SALARY +(SALARY*NVL(BONUS,0)))*12*3/100))
FROM EMPLOYEE;
...
--8
SELECT EMP_NAME, SALARY, ((SALARY + (SALARY*NVL(BONUS,0)))*12 -((SALARY + (SALARY*NVL(BONUS,0)*12*3/100)))), HIRE_DATE 
FROM EMPLOYEE
WHERE ((SALARY + (SALARY*NVL(BONUS,0)))*12 -((SALARY + (SALARY*NVL(BONUS,0)*12*3/100)))) >= 50000000;
...
--15
SELECT EMP_NAME "사원명", 
 SUBSTR(EMP_NO, 1, 1) || SUBSTR(EMP_NO, 2, 1) "생년" , 
  SUBSTR(EMP_NO, 3, 1) || SUBSTR(EMP_NO, 4, 1) "생월",
 SUBSTR(EMP_NO, 5, 1) || SUBSTR(EMP_NO, 6, 1) AS "생일"
FROM EMPLOYEE;
...
SELECT EMP_NAME, TRUNC(ABS(HIRE_DATE - SYSDATE)) AS "근무일수1",
                 TRUNC( SYSDATE - HIRE_DATE) AS "근무일수2"
FROM EMPLOYEE;

--18
SELECT *
FROM EMPLOYEE
WHERE MOD(EMP_ID, 2) = 1; --자동 형변환 해줌
...
FROM EMPLOYEE
WHERE DEPT_CODE IN ('D5', '56', 'D9')
ORDER BY DEPT_CODE;

--23
SELECT EMP_NAME, SUBSTR(EMP_NO, 1, 6) AS "앞자리", SUBSTR(EMP_NO, 8) AS "뒷자리",
