...
--1. 연결할 두 컬럼명이 다른 경우(EMPLOYEE : DEPT_COD / DEPARTMENT : DEPT_ID)
--JOIN ON
--전체 사원들의 사번, 사원명, 부서코드, 부서명
...
--2. DEPARTMENT와 LOCATION 테이블을 참고하여 전체부서의
...
--외부조인과 비교할만한 내부조인
--사원명, 부서명, 급여, 연봉
--부서배치를 받지않는 2명의 사원정보 누락됨

--1) LEFT [OUTER] JOIN : 두 테이블 중 왼편에 기술된 테이블을 기준으로 JOIN해라
--ANSI
...
--FULL [OUTER] JOIN : 두 테이블이 가진 모든 행을 조회할 수 있다. -- 오라클은 FULL을 사용 불가
SELECT EMP_NAME, DEPT_TITLE, SALARY, SALARY*12
FROM EMPLOYEE
...
--=======================================================================
/*
    3. 등가 조인(NON EQUAL JOIN)
    매칭시킬 컬럼에 대한 조건 작성시 '=' 를 사용하지 않는 조인문
    ANSI 구문 사용시 JOIN ON만 사용가능
*/
...
--전체 사원의 사원 사번, 사원명, 사원부서코드, -->  EMPLOYEE E
