/*
    ...
    [예외 처리부] : EXCEPTION으로 시작, 예외 발생시 해결하기위한 구문
...
DECLARE 
    EID NUMBER;
    ENAME VARCHAR2(20);
    PI CONSTANT NUMBER := 3.14;    --상수를 의미함
BEGIN 
    EID := 800;
    ENAME := '김용담';
...
-------------------------------------------------------------
--1_2) 래퍼런스 타입 변수 선언 및 초기화(어떤 테이블의 어떤 컬럼이 데이터 타입을 참조해서 그 타입으로 지정)
DECLARE 
...
각자료형 EMPLOYEE(EMP_ID, EMP_NAME, JOB_CODE, SALARY). DEPARTMENT(DEPT_TITLE)
등을 참조하도록 사용자가 입력한 사원의 사번, 사원명 ,직급코드, 급여 ,부서명 조회후 각변수에 담아 출력
*/
DECLARE
    EID EMPLOYEE.EMP_ID%TYPE;
    ENAME EMPLOYEE.EMP_NAME%TYPE;
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('보너스 : 0' || BONUS || '%');
END;
/
...
--  참조변수(ENP_ID, EMP_NAME, DEPT_TITLE, NATIONAL_CODE)
--일반타입변수(THEN 문자열) - 국내팀, 해외팀 분리해서 저장
--BEGIN
--사용자가 입력한 사번의 사원정보를 가져와 사번, 이름, 부서명, 근무국가코드 조회 후 각 변수에 대입
--NCODE 값이 KO 일 경우 -> THEN --> 국내팀
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('당신의 점수는 ' || SCORE || '점이며,' || GRADE || '입니다.');
END;
/
...
BEGIN
    FOR I IN REVERSE 1..5 -- REVERSE 반대로 가게 하고 싶을 때 사용
    LOOP
        DBMS_OUTPUT.PUT_LINE(I);
    END LOOP;
END;
